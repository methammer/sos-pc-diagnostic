# ============================================================
#  SOS-PC - Script de diagnostic systeme
#  Usage : irm https://sos-pc.click/diag.ps1 | iex
#  Ce script collecte des informations techniques sur votre PC
#  et ouvre automatiquement la page de diagnostic SOS-PC.
# ============================================================
...
$gpuInfo = @{
    name   = $gpu.Name
    ram_mb = [math]::Round($gpu.AdapterRAM / 1MB)
    driver = $gpu.DriverVersion
}

# --- Processus gourmands (Top 8 RAM) ---
...
foreach ($path in $regPaths) {
    if (Test-Path $path) {
        Get-ItemProperty $path | Get-Member -MemberType NoteProperty | Where-Object { $_.Name -notlike "PS*" } | ForEach-Object {
            $startupApps += $_.Name
        }
    }
}

# --- Evenements critiques (dernières 24h) ---
Write-Host "  [7/7] Journal d'evenements..." -ForegroundColor Gray
$since = (Get-Date).AddHours(-24)
$critCount = (Get-WinEvent -FilterHashtable @{LogName='System','Application'; Level=1,2; StartTime=$since} -MaxEvents 50 -ErrorAction SilentlyContinue | Measure-Object).Count
$recentErrors = Get-WinEvent -FilterHashtable @{LogName='System'; Level=2; StartTime=$since} -MaxEvents 3 -ErrorAction SilentlyContinue | ForEach-Object {
    @{ id = $_.Id; msg = ($_.Message -split "`n")[0] -replace '[^\x20-\x7E]','' | Select-Object -First 1 }
}
...
$payload = @{
    ...
    disks   = @($disks)
    procs   = @($topProcs)
    startup = @($startupApps | Select-Object -First 10)
    events  = @{ critical_24h = $critCount; samples = @($recentErrors) }
    collected_at = (Get-Date -Format "yyyy-MM-ddTHH:mm:ss")
}

# --- Encodage Base64 pour l'URL ---
$json    = $payload | ConvertTo-Json -Depth 5 -Compress
$bytes   = [System.Text.Encoding]::UTF8.GetBytes($json)
$encoded = [Convert]::ToBase64String($bytes)

# --- Ouverture du navigateur ---
$url = "https://sos-pc.click/diagnostic?d=$encoded"

Write-Host ""
Write-Host "  Collecte terminee !" -ForegroundColor Green
Write-Host "  Ouverture de la page de diagnostic..." -ForegroundColor White
Write-Host ""

Start-Process $url

Start-Sleep -Seconds 2
Write-Host "  Cette fenetre va se fermer automatiquement." -ForegroundColor Gray
