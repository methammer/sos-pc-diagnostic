...
export default async (req, context) => {
  ...
  if (!data)
    return new Response(
      JSON.stringify({ error: "Donnees systeme manquantes" }),
      { status: 400, headers },
    );
  ...
  lines.push(
    "Tu es un expert en depannage PC Windows pour SOS-PC, service de reparation informatique en France.",
  );
  lines.push(
    "Analyse les donnees systeme et reponds UNIQUEMENT en JSON valide (sans markdown ni backticks) avec cette structure :",
  );
  lines.push(
    '{"summary":"...","score":85,"issues":[{"level":"critical|warning|ok","category":"RAM|CPU|Stockage|Temperature|Demarrage|Systeme|Securite","title":"...","description":"...","action":"..."}],"quick_wins":["..."],"needs_professional":true,"professional_reason":"..."}',
  );
  ...
  lines.push(
    "OS : " +
      (d.os && d.os.name ? d.os.name : "inconnu") +
      " build " +
      (d.os && d.os.build ? d.os.build : "?"),
  );
  lines.push(
    "Arch : " +
      (d.os && d.os.arch ? d.os.arch : "?") +
      " | Uptime : " +
      (d.os && d.os.uptime ? d.os.uptime : "?") +
      "h",
  );

  const ramTotal =
    d.os && d.os.ram_total_gb ? parseFloat(d.os.ram_total_gb) : 0;
  const ramFree = d.os && d.os.ram_free_gb ? parseFloat(d.os.ram_free_gb) : 0;
  const ramPct =
    ramTotal > 0 ? Math.round((1 - ramFree / ramTotal) * 100) : "?";
  lines.push(
    "RAM : " +
      ramTotal +
      " Go total | " +
  ...
  lines.push(
    "CPU : " +
      (d.cpu && d.cpu.name ? d.cpu.name : "?") +
      " | " +
      (d.cpu && d.cpu.cores ? d.cpu.cores : "?") +
      " coeurs | charge " +
      (d.cpu && d.cpu.load ? d.cpu.load : "?") +
      "%",
  );
  lines.push(
    "GPU : " +
      (d.gpu && d.gpu.name ? d.gpu.name : "?") +
      " | " +
      (d.gpu && d.gpu.ram_mb ? d.gpu.ram_mb : "?") +
      " Mo VRAM",
  );
  ...
  lines.push(
    "DEMARRAGE : " +
      (d.startup && d.startup.length > 0 ? d.startup.join(", ") : "Aucun"),
  );
  lines.push(
    "EVENEMENTS 24h : " +
      (d.events && d.events.critical_24h ? d.events.critical_24h : 0) +
      " erreurs critiques",
  );
  ...
  try {
    const url =
      baseUrl + "/v1beta/models/gemini-2.5-flash-lite:generateContent";
